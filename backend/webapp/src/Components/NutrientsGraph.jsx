...
const NutrientsGraph = ({ nutrientData }) => {
  const chartRef = useRef(null); // Reference to the canvas element
  const chartInstanceRef = useRef(null); // Reference to the Chart.js instance

  useEffect(() => {
    if (!chartRef.current || !nutrientData || nutrientData.length === 0) return;

    // Extract the nutrients array from the data
    const nutrients = nutrientData[0]?.nutrients || [];
    ...
    const data = sortedNutrients.map((_, index) => index + 1); // Use index+1 for y-axis data
    ...
    chartInstanceRef.current = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [
          {
            label: `100 gram ${nutrientData[0].name?nutrientData[0].name:""} `, 
            data: data, // Y-axis data (index + 1)
          ...
          tooltip: {
            ...
            callbacks: {
                label: (context) => {
                    ...
                    const nutrientValue = data[context.dataIndex];
  ...
  return (
    <div style={{ width: "100%", height: `250px`}} className="p-0 d-flex flex-column justify-content-center align-items-center">
      <canvas className="" style={{ width: "100%", height: `100%`}} ref={chartRef} /> {/* Canvas element for Chart.js */}
    </div>
  );
};
